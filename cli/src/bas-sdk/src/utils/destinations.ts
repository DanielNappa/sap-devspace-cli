...
export interface OAuth2UserTokenExchange extends
  Omit<
...
export function strToTokenServiceUrlType(
  ...
  const tokenServiceUrlType = typeNameArr.find((element) =>
    element === typeName
  );
...
function getOAuthProperties(
  ...
  const oauthProperties = authenticationTypeToProperties
