...
import { PageGenieSession } from './session';
import { getServiceFilePaths, normalizeVariableValue } from '../util/mdk-util';

export class MDKPageGenie extends AbstractGenie {
  ...
  async startSession(initialPrompt: string, userContext: UserContext, activeEnv: ActiveEnv): Promise<IGenieSession> {
    const genieSession = new PageGenieSession(this, initialPrompt, userContext, activeEnv);
  ...
  async accept_json(response: string, session: IGenieSession, actionName: string, actionRunContext?: ActionRunContext): Promise<Boolean> {
    return (session as PageGenieSession).accept(response, actionName, actionRunContext);
  }
